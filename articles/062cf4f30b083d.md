...
GridCellsクラスはセルについての二次元配列のデータを保持し、そのインターフェースを提供する責務を持ちます。**座標平面についての知識はこのクラスに閉じ込めています**。例えば、配列のインデックス指定, flattenなどの処理はGridCellsクラス内でのみ行います。

**🟢 GridCellsのインターフェース**
- ２次元配列`data`は外部からアクセスできない
```ruby:grid_cells.rb
class GridCells
  ...
